...
The rule that the list of corrective action requests is always present is the part to test, and it would be relaxed if assessors could not adopt it without changing how their schemes record results, in which case an absent list would still be reported as not stated and never as none raised.
